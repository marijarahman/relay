...
const mutation = new GraphQLObjectType({
	...
	fields: {
		...
		removeUser: {
			...
			resolve(parentValues, args) {
				return axios.delete(`http://localhost:3000/users/${args.id}`).then(response => response.data)
			}
		},
		editUser: {
			type: UserType,
			...
			resolve(parentValues, { firstName, age, id }) {
				return axios.put(`http://localhost:3000/users/${id}`, {
					firstName,
					age,
				}).then(response => response.data)
			}
